...
public abstract class BaseRequest {
    ...
    protected void pocess(boolean hasEncrypt) {
        ...
        if (entity == null) {
            DebugUtil.e(TAG, "new StringEntity failed !");
            return;
        }
        if (BuildConfig.DEBUG) {
            DebugUtil.i(TAG, entity.toString());
        }
...
//		AsyncHttpClient client = getHttpClient();
        ...
        try {
            ...
            DebugUtil.i(TAG, "charset：" + charset);
            DebugUtil.i(TAG, "response status: " + response.getStatusLine());
    ...
    private StringEntity createRequestEntity(boolean hasEncrypt) {
        ...
        DebugUtil.e("RequestData", data.toString());

        // 加密RequestData
        String postString = new Gson().toJson(data);
        ResponseData.CurrentPostString=postString;

        DebugUtil.e(TAG, "请求接口 : " + getFunctionName() + " 的postString : " + "\n" + postString);
        if (hasEncrypt) {
            try {
                postString = Encrypt.encrypt(postString);
                postString = URLEncoder.encode(postString, HTTP.UTF_8);
                DebugUtil.e(TAG, "加密后的postString : " + postString);

            } catch (Exception e) {
                e.printStackTrace();
                DebugUtil.i(TAG, "encrypt exception:" + e);
            }
        }
        ResponseData.CurrentEncryptPostString=postString;
    ...
    protected HttpClient getHttpsClient() {
        if (httpsClient == null) {
            try {
                ...
                SingleClientConnManager conManager = new SingleClientConnManager(params, schReg);

                httpsClient = new DefaultHttpClient(conManager, params);
                //httpsClient=  MyHttpClient.createHttpClient();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
